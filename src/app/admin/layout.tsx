...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const NAV = [
    ...
    { href: '/admin/orders', label: t('admin.sidebar.orders'), icon: ShoppingBag },
  ]
      ...
      >
        <div className="p-5 border-b border-white/10 flex items-center justify-between">
          <Link href="/admin" className="font-bold text-lg">
            ...
            <span className="block text-[10px] text-gray-500 font-normal tracking-widest uppercase">{t('admin.sidebar.admin')}</span>
          </Link>
      ...
      <div className="flex-1 min-w-0">
        <div className="sticky top-0 z-20 bg-gray-950 border-b border-white/10 p-4 flex items-center gap-3 md:hidden">
          ...
          <span className="font-bold text-sm">
            <span className="text-white">Next</span><span className="text-blue-400">Step</span>
            <span className="text-gray-500 font-normal"> / {t('admin.sidebar.admin')}</span>
          </span>
        </div>
